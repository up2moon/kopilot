...
주중 거래가 끝나기 전에 미션을 판정하면 이후 결제로 결과가 바뀔 수 있다. 따라서 월요일부터 금요일까지 이번 주 미션 전체를 함께 수행하고, 토요일 00:00 KST부터 해당 기간의 전체 거래내역으로 한 번에 인증한다.
...
- 개발 시연에서는 `CHALLENGE_TEST_NOW`에 타임존 오프셋을 포함한 ISO 8601 시각을 지정해 챌린지 기준 시각만 오버라이드할 수 있다. 운영 환경에서는 이 값을 무시한다.
...
`Authorization: Bearer <accessToken>`이 필요하며 요청 본문은 없다. 현재 주의 `IN_PROGRESS` 미션 전체를 동일한 거래내역 스냅샷으로 일괄 판정한다.
...
- 완료 후 `성공 개수/전체 개수`, 획득 포인트, 각 미션 결과를 함께 표시한다.
