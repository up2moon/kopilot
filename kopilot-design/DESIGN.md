...
- 하단 메뉴바는 `홈`, `로드맵`, `챗봇`, `마이` 4개 탭으로 구성한다.
- 챗봇은 하단의 독립 탭으로 제공하므로 플로팅 챗봇 버튼은 표시하지 않는다.
...
- 가변 개수의 주간 챌린지를 한 화면에 함께 보여주고, 각 챌린지는 기준 거래내역과 이번 주 목표를 하나의 구체적인 문장으로 표시한다. AI 채팅에서 방금 추가된 카드는 자동 스크롤, 강조 테두리, `방금 추가됨` 배지로 안내한다. 유사한 제목과 설명을 중복 노출하지 않는다.
- 인증 전에는 `진행중`을 포함한 상태 배지를 숨기고, 인증 결과가 확정된 뒤에만 `성공` 또는 `미완료`를 표시한다.
- 프로토타입에서는 미인증 챌린지가 있으면 요일과 시각에 관계없이 `인증하기` 버튼을 활성화한다.
- 인증 결과에는 성공 개수, 포인트와 예상 절약액을 함께 표시해 보상감을 강화한다.
- 성공 결과에는 `투자효과 보기` CTA를 표시하고, 사용자가 누르면 해당 월의 `절약액` 탭으로 이동한다.
- 인증 성공 시 결과 요약이 먼저 읽히도록 하고 그 위에 짧은 컨페티 효과를 재생한다. 컨페티가 CTA나 결과 문구를 가리지 않게 하며, 모션 감소 설정에서는 정적인 성공 상태로 대체한다.

## 투자효과 화면 규칙

- 소비 카테고리 탭의 첫 항목으로 `절약액`을 제공한다.
- `절약액`은 선택 월에 성공한 챌린지의 예상 절약액 합계를 강조하고 소비액과 다른 문구로 설명한다.
- 절약액이 없으면 오류 카드 대신 챌린지 인증을 안내하는 빈 상태와 `챌린지 보러 가기` CTA를 표시한다.
