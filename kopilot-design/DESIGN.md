...
- 가변 개수의 주간 미션을 한 화면에 함께 보여주고, 각 미션은 기준 거래내역과 이번 주 목표를 하나의 구체적인 문장으로 표시한다. AI 채팅에서 방금 추가된 카드는 자동 스크롤, 강조 테두리, `방금 추가됨` 배지로 안내한다. 유사한 제목과 설명을 중복 노출하지 않는다.
- 인증 전에는 `진행중`을 포함한 상태 배지를 숨기고, 인증 결과가 확정된 뒤에만 `성공` 또는 `미완료`를 표시한다.
- 프로토타입에서는 미인증 미션이 있으면 요일과 시각에 관계없이 `인증하기` 버튼을 활성화한다.
