...
- `POST /api/users/me/saving-bot/investment-analysis`: 선택 종목 코드, 이름, 월 절약액을 받아 코스콤 시세와 Naver/Firecrawl 뉴스 검색 결과를 병렬 수집하고, `risk_agent`와 `opportunity_agent`의 찬반 근거 및 `report_agent`의 장기 자산 시나리오를 JSON으로 반환합니다.
- `POST /api/users/me/saving-bot/investment-analysis/stream`: 위 종목 분석을 SSE로 실행합니다. `information.started/completed`, `perspectives.started`, 에이전트별 `perspective.delta/completed`, `report.started`, `analysis.completed` 순서의 이벤트를 보냅니다. 찬반 에이전트는 병렬 실행되며 각 에이전트의 근거 문장이 생성되는 동안 `perspective.delta`로 동시에 스트리밍되고, 완료되는 순서대로 확정 근거 카드가 표시됩니다.
...
OpenAI 기반 합성 거래내역 생성과 절약 챗봇은 환경 변수 `OPEN_AI_KEY`를 사용합니다. 모델은 기본 `gpt-4.1-mini`이며 `OPENAI_MODEL`로 변경할 수 있습니다. 절약 챗봇 File Search에는 `OPENAI_SAVING_VECTOR_STORE_ID`가 추가로 필요합니다. 종목 분석 에이전트는 `OPENAI_INVESTMENT_AGENT_MODEL`이 있으면 해당 모델을 우선 사용하고, 없으면 `OPENAI_MODEL`을 사용합니다. 뉴스 수집은 백엔드에 복제한 구현이 아니라 저장소의 `news_agent/tools.mjs`를 직접 import하며 루트 `.env`의 `NAVER_API_CLIENT_ID`, `NAVER_API_SECRET_KEY`, `FIRECRAWL_API_KEY`를 사용합니다. 사용자가 먼저 지정한 오탈자 `FIRECRALW_API_KEY`도 호환하지만 새 환경은 표준 이름을 권장합니다. Naver와 Firecrawl은 각각 종목 최신 소식·실적·주가 전망·성장 투자·리스크 악재의 5개 검색어를 실행합니다. Naver 검색은 병렬 실행하고 Firecrawl은 요청 제한을 피하도록 2개씩 실행하며, 진행 상황을 SSE `information.progress` 이벤트로 전송합니다. 최근 `INVESTMENT_NEWS_LOOKBACK_DAYS`일(기본 90일) 내 결과를 직접 근거·기업 관련·보조 근거·산업 참고로 등급화하고 기본 최대 24건을 찬반 에이전트에 전달합니다. 관점별 에이전트는 정보가 충분하면 최대 8개의 중복되지 않는 근거를 생성합니다. 검색량은 `INVESTMENT_NEWS_NAVER_DISPLAY`, `INVESTMENT_NEWS_FIRECRAWL_LIMIT`, `INVESTMENT_NEWS_MAX_RESULTS`로 조정합니다. 허용 언론사 도메인은 수집 후 공통 검증 단계에서 적용하며 `INVESTMENT_NEWS_ALLOWED_DOMAINS`로 대체할 수 있습니다. 일부 검색이 실패해도 성공한 검색 결과로 분석을 계속합니다.
