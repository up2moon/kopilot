...
- `GET /api/investment/assets/search`: DB에 적재된 코스콤 CHECK API 종목/ETF 마스터(`investment_asset`) 목록을 최대 20개 기본 반환하며, 선택적인 `keyword`로 서버 검색할 수도 있습니다. 각 종목은 DB에 저장된 최신 종가와 전일 대비 등락률을 `currentPrice`, `diffRate`로 함께 반환하고, DB가 비어 있으면 최초 요청에서 코스콤 마스터를 적재합니다.
