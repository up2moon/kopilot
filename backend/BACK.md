...
- `GET /api/users/me/saving-bot/chat/history`: Redis에 저장된 현재 사용자의 최근 AI 절약 챗봇 대화를 반환합니다. 자산배분 답변은 텍스트와 구조화된 `assetAllocation`을 함께 저장·반환하므로 채팅 페이지 재진입 후에도 배분 카드가 복원됩니다. 대화는 사용자별 List에 최대 100개까지 저장되며 마지막 대화 이후 기본 3일 동안 유지됩니다.
...
- 개발 프론트의 모든 `/api` 요청은 로컬 로그인 토큰 검증을 위해 로컬 백엔드로 전달합니다. 로컬 백엔드에서 발생한 실제 CHECK 요청만 코스콤에 직접 연결하지 않고 `https://kospay.p-e.kr/api/investment/check-proxy`를 거쳐 운영 WAS가 대신 호출합니다. 운영 프록시는 기존 CHECK 인증정보를 검증하고 사전에 허용된 CHECK 경로만 전달하여 코스콤에 보이는 호출 IP를 운영 환경으로 고정합니다. 별도의 로컬/프록시 전환 환경변수는 사용하지 않으며 `NODE_ENV=production`인 운영 WAS만 CHECK API에 직접 연결합니다.
