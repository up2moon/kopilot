...
`First Login Page`는 사용자가 Kospay에 처음 로그인했을 때 등장하는 초기 설정 페이지이다.
...
사용자가 Kospay의 목적을 이해하고 초기 설정을 시작하도록 유도한다.
...
사용자가 자신의 결제 내역을 Kospay과 연결하는 과정이다.
...
마이데이터 연동이 완료된 경우, Kospay은 최근 소비 내역을 기반으로 추천 예산을 제안한다.
...
| 버튼명      | 동작                           |
| -------- | ---------------------------- |
| 추천 예산 적용 | Kospay 추천 금액을 각 카테고리에 자동 입력 |
...
```text
docs/features/first-login.md 문서를 기준으로 Kospay의 첫 로그인 온보딩 페이지를 구현해줘.
