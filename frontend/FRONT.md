...
- `/challenge`: AI 주간 챌린지 페이지 (하단 탭 3번째). `GET /api/users/me/challenges`로 오늘의 미션과 월~금 미션 목록을 조회한다. 월요일에는 백엔드가 사용자별 AI 미션 5개를 생성·저장하며, 이후 새로고침에도 같은 미션을 표시한다. 마이데이터 또는 선택 카테고리가 없으면 온보딩 안내 상태를 표시한다. 수행 인증·성공/실패·포인트 지급은 후속 기능으로 분리되어 현재 버튼은 비활성 상태다.
