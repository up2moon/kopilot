...
- `/challenge`: AI 주간 챌린지 페이지 (하단 탭 3번째). `GET /api/users/me/challenges`로 오늘의 미션·월~금 목록·주간 진행률을 조회한다. 오늘 카드에는 소비 진행률과 다음 날 인증 안내만 표시한다. 전날 `IN_PROGRESS` 미션은 주간 목록에서 `인증하기` 버튼을 표시하며, `POST /api/users/me/challenges/:challengeId/verify`가 전날 전체 거래내역으로 성공·실패 및 포인트를 즉시 확정한다. 인증 요청 중에는 해당 미션 행에 거래내역 확인 오버레이를 표시한다.
