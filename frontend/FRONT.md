...
- `/dashboard`: 대시보드가 개발되기 전까지 첫 로그인 설정 완료 화면을 표시합니다. 첫 로그인 초기 설정이 완료되지 않은 사용자는 `/first-login`으로 이동합니다.

첫 로그인 설정이 완료된 사용자가 `/first-login` 또는 `/first-login/mydata-connect`에 직접 접근하면 `/dashboard`로 이동합니다.

첫 로그인 완료 화면과 `/dashboard` 임시 완료 화면에는 마이데이터 연동으로 저장된 결제내역을 확인하는 버튼을 제공합니다.

정의되지 않은 경로와 `/` 진입은 `/login`으로 정규화합니다.
