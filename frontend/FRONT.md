...
- `/challenge`: AI 주간 챌린지 페이지 (하단 탭 3번째). `GET /api/users/me/challenges`로 오늘의 미션과 월~금 미션 목록을 조회한다. 현재 주에 미션이 없으면 월요일 또는 그 이후 어느 요일의 첫 요청에서도 백엔드가 사용자별 AI 미션 5개를 생성·저장하며, 이후 새로고침에도 같은 미션을 표시한다. 관심 카테고리를 선택했으면 해당 카테고리만, 선택하지 않았으면 전체 소비 카테고리를 후보로 사용한다. 오늘보다 이전인 진행중 미션은 `미완료`로 표시한다. 마이데이터 거래가 없으면 안내 상태를 표시한다. 수행 인증·성공/실패·포인트 지급은 후속 기능으로 분리되어 현재 버튼은 비활성 상태다.
